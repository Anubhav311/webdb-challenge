...
RDBMS stands for Relational Database Management System. It’s a type of database management software which enables all storying, querying, and updating the database.
SQL stands for Structured Query Language. It’s a language used to communicate with an RDBMS. 
So the biggest difference between two is that SQL is a query language and RDBMS is a type of data management system.

1. Why do tables need a `primary key`?

A database table must always have rows that are unique in some way. There should be, therefore, a unique key that ensures this uniqueness and which can be used to retrieve individual rows. Unique keys make it easier and efficient to interact with the database.

1. What is the name given to a table column that references the primary key on another table.

A column named foreign_key is used to reference the primary key on another table.

1. What do we need in order to have a _many to many_ relationship between two tables.

In order to have a many to many relationship between two tables we need to create a third table. 
